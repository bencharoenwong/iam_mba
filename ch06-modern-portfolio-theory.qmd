---
abstract: "This chapter formalizes the principles of portfolio selection introduced by Harry Markowitz, focusing on the efficient frontier and the Capital Allocation Line (CAL). It explains the 'tangency portfolio' and the counterintuitive result that risk tolerance determines only the allocation to risky assets, not the choice of risky assets themselves."
keywords: ["Modern Portfolio Theory", "Efficient Frontier", "Sharpe Ratio", "Tangency Portfolio", "Capital Allocation Line", "Risk-Free Asset"]
---

# Session 06: Modern Portfolio Theory and Its Evolution

## Guiding Questions
- How has portfolio theory evolved to address real-world challenges?
- What are the practical limitations of mean-variance optimization?
- How do practitioners implement portfolio theory in the real world?
- What innovations have improved upon traditional portfolio theory?

::: {.content-visible when-profile="course"}
::: {.callout-important}
## Major Project: Optimal Portfolios
Following Session 10, you will be assigned the **Optimal Portfolios Project**. This week-long assignment requires you to apply the mean-variance optimization and Capital Allocation Line concepts covered in this chapter and Session 5 to construct and justify an optimal portfolio using real market data.
:::
:::
...
We are now formalizing the diversification intuition from Chapter 5 into a framework you can actually use. The math is standard — mostly linear algebra and some calculus — but the economic logic underneath it is what you need to internalize.
...
Tom would optimally use 30% leverage — borrowing to invest 130% in equities. This is the theoretical justification for leveraged portfolios, though in practice borrowing costs reduce the attractiveness (as we will see with leveraged ETFs in Chapter 8).
:::
...
**The 1/N puzzle.** Perhaps the most humbling result: a naive equal-weight portfolio often outperforms sophisticated optimization out of sample. The details of this finding, and what it implies for practice, are worth examining (see the case study below).

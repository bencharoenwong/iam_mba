---
abstract: "This chapter evaluates the ongoing debate between active and passive investment management through the lens of the Capital Asset Pricing Model (CAPM) and the two-fund separation theorem. It explains why the market portfolio is a powerful benchmark and discusses how heterogeneity in investor constraints and risk capacity allows market dislocations to persist."
keywords: ["Active Management", "Passive Management", "CAPM", "Market Portfolio", "Alpha", "Beta"]
---

# Session 07: Active vs. Passive Management

## Guiding Questions
- Should we expect active management to add value?
- How should investors evaluate active manager performance?
- What role should passive investments play in portfolios?
- How is the active-passive debate evolving with new investment vehicles?
...
To understand the active-passive debate, we need to start where the last chapter ended: portfolio theory and what it implies about how rational investors should allocate capital.

As we saw in the previous chapter, mean-variance optimization applied to twenty years of historical data points overwhelmingly toward US equities — a result driven largely by the extraordinary post-2008 run.
...
**Passive management** — also called indexing — is a long-term buy-and-hold strategy whose objective is to track a benchmark index with minimum deviation. The defining metric is tracking error: how far does the portfolio's return stray from the benchmark? Fees can be as low as 3–10 basis points for broad equity index funds, though more specialized index products can cost 20 basis points or more, and portfolio turnover is low, around 2% per year, because holdings change only when the index itself changes.

**Active management** sets out to outperform a passive benchmark on a risk-adjusted basis. Higher fees — ranging from roughly 0.7% to 1.0% for actively managed mutual funds, and up to 2% for hedge funds — and higher turnover, roughly 75% per year, reflect the research, trading, and analyst infrastructure required. The objective is not to minimize tracking error but to generate alpha: return above the benchmark after adjusting for the risk taken.
...
With the CAPM as a benchmark, we can ask the central question of this chapter directly: does active management add value?
...
The equilibrium result, formalized by Berk and Green (2004), is that funds with genuine alpha attract inflows until the alpha is exactly equal to the management fee. Investors earn zero net alpha in expectation, even though some managers have real skill. The skilled managers capture their skill in the form of fees, not in excess returns to investors. In other words, established funds with high fees generally show no alpha — but add back the management fee and that is their alpha. They are earning their keep, because if they were not, the fund would disappear.

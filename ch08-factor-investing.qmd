...
The second is the expense ratio. The SPY — the most liquid security in the world — charges about 9 basis points annually. The triple-leveraged ETF charges roughly 75 basis points. That fee is not paid quarterly. It accrues daily into the net asset value, which means the drag is continuous. Multiply the monthly alpha estimate by 12 and you recover approximately the expense ratio: the negative alpha is not a failure of execution — it is the fee.

::: {.callout-important}
## Common Misconception

Volatility drag is often cited as the primary explanation for why leveraged ETFs underperform their stated multiple over time. That explanation applies to the compound annual growth rate (CAGR) but not to simple average returns. The CAGR will be worse than the arithmetic average because of volatility drag, but the regression beta is estimated on simple returns — so the gap between 2.8 and 3.0 is explained primarily by borrowing costs and fees, not by the arithmetic-versus-geometric return distinction.
:::
...
Mark Carhart, then at the University of Chicago, added a fourth factor in 1997: momentum. Stocks that have performed well over the past twelve months, excluding the most recent month, tend to continue outperforming — a pattern that has replicated across asset classes and international markets.
...
Consider the analogy: a large Indonesian cigarette conglomerate — the hand-rolled, loose-leaf kind — is deeply exposed to the risk that vaping and nicotine pouches replace traditional tobacco consumption. A technology company benefits from that same disruption. The conglomerate is a value stock. It trades cheap because the market implicitly prices in the risk that its business model becomes obsolete. If you hold value stocks, you are holding a portfolio of companies that are negatively exposed to technology progress. The value premium is what the market pays you to bear that exposure.
...
The risk-based account of factor premiums has a formal theoretical home. Robert Merton's Intertemporal Capital Asset Pricing Model (ICAPM), for which he shared the 1997 Nobel Prize, is the equilibrium framework that explains why multiple factors belong in expected return equations even in a world of rational investors.
...
> "I was at a conference talking to classmates. 'Yeah, I'm going to start a machine learning fund.' Good luck. You think the Sharpe ratio of two is real? You learn very fast when you have to do the apology tour for your investors."

The honest appraisal is that machine learning is genuinely useful for the ensemble problem — learning weights across a large set of well-specified signals — and much less useful for discovering new signals that will survive out of sample. The signals themselves still require economic intuition to survive.

## How Fee Structures and Jargon Exploit Rather Than Inform

Every concept covered in this session has a direct implication for how investors are charged — and how those charges can be disguised.

The leverage ETF example is clear: you are paying 75 basis points annually for an asset that could be replicated by an allocation decision in your existing portfolio. The fee is not nothing — the product does provide genuine convenience — but the cost-benefit calculation should be made explicitly.

---
abstract: "Examining the intersection of artificial intelligence and finance, this chapter defines complexity through the lens of machine learning and the bias-variance tradeoff. It explores how sophisticated models can generate economic value even with low statistical accuracy, while cautioning against the risks of overfitting in-sample noise."
keywords: ["AI in Finance", "Machine Learning", "Complexity", "Bias-Variance Tradeoff", "Quantitative Finance", "Algorithmic Trading"]
---

# Session 14: Impact of AI on Asset Management

## Guiding Questions
- How is AI changing investment decision-making?
- What are the opportunities and limitations of alternative data?
- How will technology reshape the role of investment professionals?
- What skills will future investment professionals need to succeed?

Marc Andreessen declared in 2011 that software is eating the world. Satya Nadella — a Chicago Booth graduate, incidentally — recently declared that SaaS is dead, implying something is eating software. Hype or not, the underlying observation matters for finance: the tools we use to manage money are being rebuilt from scratch, and that has consequences for every concept we have covered in this course. The question we end on is deceptively simple: can machines invest?

The short answer is that machines can do parts of investing extremely well and other parts catastrophically badly, and the industry is still figuring out which is which. This chapter ties together everything — market efficiency, factor investing, the principal-agent problem, behavioral biases — through the lens of what happens when you throw a trillion-dollar industry at modern AI.

## What We Mean by Complexity

Before we can evaluate machine learning in finance, we need a precise definition of complexity. **One useful way to measure complexity is the ratio of model parameters to available data points**. A simple regression with five predictors and five thousand observations has a complexity ratio of 0.001. A neural network with ten million parameters trained on five thousand observations has a complexity ratio of two thousand. Both are "models" in the statistical sense. They behave very differently.
...
NLP and ML models have reshaped how institutional investors process information. But the same technology wave also transformed the other end of the market — how ordinary investors access financial advice and portfolio management.
...
Machine learning adds genuine value in specific areas of asset management, but the promises outrun the reality in others.
...
*Low signal-to-noise ratio.* Finance has among the worst signal-to-noise ratios of any domain where ML is applied. Cross-sectional return variation is dominated by noise — even the best predictors explain only a few percent of variance. ML does not solve this problem; it just exploits it more efficiently.
...
AI does not invalidate anything covered in this book — it sharpens it. Market efficiency, factor investing, behavioral biases, and the principal-agent problem all still govern how money is managed. What changes is the speed and scale at which these forces operate. That makes this a natural point to step back and ask what the entire course adds up to.
